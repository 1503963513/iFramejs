...
class Iframe {
  ...
  constructor(options) {
    ...
    this.Whitelist = [];
    this._originCache = new Set(); // 缓存白名单的origin
    this._eventHandlers = new Map(); // 存储实例级别的事件处理器
    this._messageListener = null; // 存储消息监听器引用
    this._isDestroyed = false; // 标记实例是否已销毁
    this._iframeLoaded = false; // 标记 iframe 是否已加载
  ...
  _targetSend(target, payload, options) {
    ...
    try {
      ...
      if (!targetOrigin || targetOrigin === '*') {
        console.warn('[Security] Sending message to "*" is not recommended for sensitive data.');
      }
